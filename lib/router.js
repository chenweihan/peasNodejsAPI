...
var router={};   
    ...
    router.dispatcher = function(str) {
        if (typeof(str) !== 'undefined') { 
            ...
            if (arr.length == 4) {
                ...
                //执行函数 
                if (typeof(apiModule[arr[3]]) !== 'rest') {
                    router.rest(apiModule,router.req,router.res);
                } else if (typeof(apiModule[arr[3]]) !== 'undefined') {
    ...
    router.rest = function(module,req,res) {
        switch (req.method) { 
               case 'GET' :
                    module.restRead(req,res); 
               break;
               case 'POST':
                    module.restCreate(req,res); 
               break;
               case 'PUT' :
                    module.restUpdate(req,res); 
               break;
               case 'DELETE':
                    module.restDelete(req,res); 
               break;
               default:
    ...
    exports.socket = function(io) {
        ...
        //引入socket的模块的可以根据自己的需求引入
        // 常规请求 http://127.0.0.1:8081
        io.sockets.on('connection', function (socket) {
              var socketModule = require(global.ENV.projectDir+'socket/socket.js');
                  socketModule['all'](socket);
        });
          
        // '/chat'请求的  http://127.0.0.1:8081/chat
